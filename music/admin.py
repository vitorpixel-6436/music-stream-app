...
class MusicFileAdmin(admin.ModelAdmin):
    list_display = (
        ...
        'duration_display', 'play_count_badge', 'audio_preview', 'actions'
    )
    ...
    def actions(self, obj):
        return format_html(
            '<a class="button" href="{}" target="_blank" style="background: #1db954; '
            'color: white; padding: 6px 12px; border-radius: 6px; text-decoration: none; '
            'display: inline-block; font-size: 12px;">▶ Play</a>',
            reverse('admin:music_musicfile_change', args=[obj.pk])
        )
...
class SystemSettingsAdmin(admin.ModelAdmin):
    fieldsets = (
        ...
        ('Statistics (Read-only)', {
            'fields': ('total_tracks', 'total_plays', 'total_downloads'),
            'classes': ('collapse',)
        }),
    )
