"""Enhanced Django Admin Panel for Music Stream App
...
class MusicStreamAdminSite(admin.AdminSite):
    ...
    def index(self, request, extra_context=None):
        """Enhanced index page with statistics"""
        ...
        extra_context['stats'] = {
            ...
            'total_plays': MusicFile.objects.aggregate(
                Sum('play_count')
            )['play_count__sum'] or 0,
            'total_downloads': MusicFile.objects.aggregate(
                Sum('download_count')
...
class ArtistAdmin(admin.ModelAdmin):
    list_display = (
        'name', 'photo_preview', 'track_count', 
...
class AlbumAdmin(admin.ModelAdmin):
    list_display = (
        ...
        'cover_preview', 'track_count', 'created_at'
    )
...
class MusicFileAdmin(admin.ModelAdmin):
    ...
    def extract_metadata(self, request, queryset):
        ...
        self.message_user(
            request, 
            f"Metadata extracted for {queryset.count()} tracks."
        )
    extract_metadata.short_description = "Re-extract metadata"


# ============================================================================
# System Settings Admin (Singleton)
# ============================================================================
    ...
    fieldsets = (
        ('General Settings', {
            'fields': (
                ...
                'max_upload_size', 'allowed_formats'
            )
        }),
        ('User Management', {
            'fields': (
        ...
        ('Audio Processing', {
            'fields': (
                'auto_extract_metadata', 'auto_generate_waveforms', 
                'normalize_audio'
            )
        }),
        ('UI Settings', {
        ...
        ('Statistics (Read-only)', {
            'fields': (
                'total_tracks', 'total_plays', 'total_downloads',
                'created_at', 'updated_at'
            ),
            'classes': ('collapse',)
        }),
...
class UploadSessionAdmin(admin.ModelAdmin):
    list_display = (
        'id_short', 'user', 'status_badge', 
        'progress', 'created_at', 'duration'
    )
    list_filter = ('status', 'user', 'created_at')
    search_fields = ('user__username', 'id')
    readonly_fields = (
        'id', 'user', 'total_files', 'successful_uploads', 
        'failed_uploads', 'status', 'error_log', 
    ...
    def progress(self, obj):
        ...
        return format_html(
            ...
            '<div style="background: #1db954; width: {}%; height: 100%; '
